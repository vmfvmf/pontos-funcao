...
import { Coluna, Tabela } from 'src/app/tabelas/tabela';
...
export class ContagensDetalhesComponent implements OnInit {
  ...
  salvarTabelas(tabelas: Tabela[]) {
    this.tabelaService.salvaEmLote(tabelas).subscribe(
      (response: Tabela[]) => {
        console.log("Response do objeto Tabelas", response);
        tabelas.forEach(t => {
          t.id = response.find(tab => tab.nome == t.nome).id;
          t.colunas.forEach(c => {
            c.tabela = new Tabela({ id: t.id });
          });
          this.salvarColunas(t.colunas);
        });
      },
      (error) => {
  ...
  salvarColunas(colunas: Coluna[]) {
    this.colunaService.salvaEmLote(colunas).subscribe(
      (response) => {
        colunas.forEach(c => {
          c.id = response.find(col => col.nome == c.nome).id;
        });
    ...
    if (arquivo.id == undefined) {
      this.funcaoDadosServico.novo(arquivo).subscribe(
        (response) => {
          ...
          arquivo.tabelas.forEach(t => {
            t.funcaoDados = new FuncaoDados({ id: arquivo.id });
          });
          this.salvarTabelas(response.tabelas);
        },
        (error) => {
      ...
      this.funcaoDadosServico.editar(arquivo).subscribe(
        (response: FuncaoDados) => {
          ...
          this.salvarTabelas(arquivo.tabelas);
        },
        (error) => {
          this.msgService.error('Ocorreu um erro ao salvar!');
